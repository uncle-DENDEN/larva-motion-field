...
def avfill(inputfile, outputfile, centerfile, index):
    """
    Args:
        inputfile(string): path of the raw data
        outputfile(string): path of the output data
        centerfile(string): path of the center file
        index(int): corresponding row number of the trajactory in center.csv
    """
    ...
    # import target coordinate and threshold
    center = pd.read_csv(centerfile, sep=',', index_col=0)
    ce = center.iloc[index - 1, 0:6].str.split(',', expand=True).values.astype(np.float)
    thre = center.iloc[index - 1, 6]
...
avfill(r'E:\workspace\tracking\tracking_output\reinforcement 2\output\8-track.txt',
       r'E:\workspace\tracking\tracking_output\reinforcement 2\track_filled\8-track_filled.txt',
       r'E:\workspace\tracking\tracking_output\reinforcement 2\zone_file\center.csv', 8)
